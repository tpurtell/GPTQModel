# Adapted upstream regressions for this fork's integrated target+MTP loader.
...
from gptqmodel.models.definitions.glm5_next import Glm5NextQModel, _attach_glm5_next_mtp
...
def test_glm5_next_module_tree_preserves_fork_quantization_boundary():
    ...
    # This fork allows these projections; the EXL3 recipe restricts actual
    # work to routed experts. MTP is instantiated and calibrated, not copied.
    assert {"self_attn.q_a_proj", "self_attn.kv_a_proj_with_mqa",
            "mlp.shared_experts.gate_proj"} <= flat
    assert not Glm5NextQModel.out_of_model_tensors
...
def test_glm5_next_defuser_registry_preserves_expert_forward():
    ...
    class TinyOuter(nn.Module):
        def __init__(self, language_model):
            super().__init__()
            self.config = SimpleNamespace(model_type="glm5_next", text_config=language_model.config)
            self.model = nn.Module()
            self.model.language_model = language_model

    model = _attach_glm5_next_mtp(TinyOuter(text_model))
    ...
    with torch.no_grad():
        actual = model.model.language_model.layers[3].mlp.experts(
...
def test_glm5_next_lazy_turtle_materializes_defused_experts(tmp_path):
    ...
    with torch.device("meta"):
        shell = Glm5NextQModel.loader.from_config(config).eval()
